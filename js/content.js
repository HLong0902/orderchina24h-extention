...
// var domain_addcart="http://localhost:8057/cart/addtocart/";
...
function renderbox(price_result)
{
	...
	//html toan bo addon
	var s = '<div>'+
		'<dl class="clearfix" id="gia-tinphat" style="font-size: 16px;margin: 10px;max-width: 500px;border-width: 3px;border-style: solid;border-color: rgb(0, 124, 194);border-image: initial;overflow: hidden;background: rgba(199, 146, 46, 0.208);">'+
		'<h5 class="tool_title" style="text-align: center;color: rgb(255, 255, 255);text-transform: uppercase;font-family: Arial;font-size: 20px;font-weight: 400;background: rgb(0, 124, 194);padding: 4px 0px 7px;margin: 0px 0px 7px;">Hàng quảng châu 24h Order Tool</h5>'+
		'<div class="taobaovn-info-inner" style="padding: 0px 10px 10px;overflow: hidden;"><img src="https://hangquangchau24h.vn/css/logo.png" alt="Hàng quảng châu 24h" style="width: 160px;float: left;margin: 8px 15px 0px 0px;"><ul style="float: left;padding: 0px;"><li style="font-size: 14px;font-family: Arial;color: rgb(0, 0, 0);margin-left: 0px !important;list-style: none !important;">Giá bán: <b class="taobaovn-rate taobaovn-color-price" style="color: rgb(211, 26, 26);font-weight: bold;font-size: 18px;">'+(price_result)+'</b> VNĐ '+
...
'</div>';
	s +='<style>.addon-wrapper{font: 12px/1.5 tahoma,arial,"Hiragino Sans GB","\5b8b\4f53",sans-serif;display: inline-block;text-align:center;width: 100%;float: left;font-family: Helvetica, Arial, sans-serif;position: fixed;bottom: 0;left: 0;z-index: 99999999999999999999999999999999999999;background: #242935;color: #fff;opacity: 0.9;}'+
		...
		'#hideAddon {position: absolute;right: 0px;width: 32px;height: 32px;background: url(https://hangquangchau24h.vn/images/add_on/hide.png) no-repeat center right;	z-index: 999999999999;bottom: 35px;}'+
...
function taobao(cart_url,url_save) {
    ...
    function getTitle()
    {
        ...
        if(document.getElementsByClassName("ItemHeader--mainTitle--3CIjqW5").length>0)
        {
            tb=document.getElementsByClassName("ItemHeader--mainTitle--3CIjqW5")[0].textContent;
        }
                ...
                if(imgElement.length > 0){
                    for(i = 0; i < imgElement.length; i++){
                        if(imgElement[i].getAttribute('class') != null && imgElement[i].getAttribute('class') != "" 
                            && imgElement[i].getAttribute('class').includes('PicGallery--mainPic')){
                            img_src = imgElement[i].getAttribute("src");
                            break;
                        }
                    }
                }
    ...
    function getLink()
    {
        ...
        if(seller_id == ""){
            ...
            if (allSpan.length > 0) {
                for (var i = 0; i < allSpan.length; i++) {
                    if (allSpan[i].getAttribute('class') != null && allSpan[i].getAttribute('class') != '' && allSpan[i].getAttribute('class') != undefined) {
                        if (allSpan[i].getAttribute('class')
                            .includes('PicGallery--thumbnailPic') || allSpan[i].getAttribute('class')
                            .includes('ShopHeader--pic')) {
        ...
        if(seller_id == ""){
            ...
            if(allDiv.length > 0){
                for(var i = 0; i < allDiv.length; i++){
                    if(allDiv[i].getAttribute('class') != null && allDiv[i].getAttribute('class') != '' && allDiv[i].getAttribute('class') != undefined){
                        if (allDiv[i].getAttribute('class').includes('ShopHeader--board'))
                        {
                            if(allDiv[i].href != ''){
                                seller_id = encodeStrToNumber(allDiv[i].href);
                            }
                        }
                    }
                }
        ...
        if(seller_id == ""){
            ...
            if(host != "detail.tmall.com" && host != "detail.tmall.hk"){
                 seller_id = encodeStrToNumber(window.location.href);
            }
        ...
        }else if(document.getElementsByClassName('mui-amount-input').length>0) //tmall
        {
           ...
           element=element[0];
           quantity = element.value; 
        } else if(document.getElementsByClassName('countValueForPC').length>0) {
            quantity=document.getElementsByClassName('countValueForPC')[0].value;
        }
        else quantity = ''; 
        //lay title
        var title=getTitle();
